...
namespace Functional.CQS.AOP.IoC.PureDI.Caching
{
	...
	public class AsyncQueryHandlerCachingDecoratorForValueResultType<TQuery, TResult> : IAsyncQueryHandler<TQuery, TResult>
		...
		where TResult : struct
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="AsyncQueryHandlerCachingDecoratorForValueResultType{TQuery, TResult}"/> class.
		/// </summary>
		...
		public AsyncQueryHandlerCachingDecoratorForValueResultType(
		...
		public async Task<TResult> HandleAsync(TQuery query, CancellationToken cancellationToken = new CancellationToken())
		{
			...
			var result = await _cache.GetAsync(cacheKey, groupKey, typeof(TResult), async () =>
			{
				isCacheHit = false;
				return await _queryHandler.HandleAsync(query, cancellationToken);
			}, input => _cachingStrategy.ShouldCacheResult((TResult)input), _cachingStrategy.TimeToLive)
				.Select(obj => (TResult)obj);
